...
export async function onRequestPost(context) {
  try {
    ...
    const { token, dni, respuestas } = await request.json();

    if (!token || !dni || !respuestas) {
      return buildJson({ status: 'ERROR', message: 'Token, DNI y respuestas son requeridos' }, 400);
    }
    ...
    return buildJson({ status: 'ERROR', message: error?.message || 'No se pudo guardar la encuesta' }, 500);
  }
}
