...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { token, respuestas } = body;

    if (!token || !respuestas) return json(res, 400, { status: "ERROR", message: "Token y respuestas son requeridos" });
    ...
    return json(res, 500, { status: "ERROR", message: error?.message || "No se pudo guardar la llamada" });
  }
}
